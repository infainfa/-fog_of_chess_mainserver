  ...
  socket.on('make_move', ({ gameId, from, to, promotion }) => {
    ...
    for (const [id, fogColor] of [[players.white, 'w'], [players.black, 'b']]) {
      ...
      if (s) s.emit('move_made', {
        ...base,
        board:          isGameOver ? fullBoard : fog.filterBoard(fullBoard, fogColor),
        visibleSquares: [...fog.getVisibleSquares(fullBoard, fogColor)],
        fullBoard:      isGameOver ? fullBoard : null,
      });
    }
